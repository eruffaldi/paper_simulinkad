...
def deepnn(x,filter1_size=5,features1=32,filter2_size=5,features2=64,densesize=1024,classes=10):
  ...
  # 7 is imagewidth/4
  W_fc1 = weight_variable([8 * 8 * features2, densesize])
  b_fc1 = bias_variable([densesize])
    
  h_pool2_flat = tf.reshape(h_pool2, [-1, 8*8*features2])
...
def main(_):
  ...
  if False:
    ...
    gen = ImageDataGenerator()
    test_gen = gen.flow(X_train, y_train, batch_size=batch_size, seed=0)
    ...
    for i in range(iterations):
      ...
      if False and i % 100 == 0:
        train_accuracy = accuracy.eval(feed_dict={
            x: xs, y_: ys, keep_prob: 1.0}, options=run_options, run_metadata=run_metadata)
...
if __name__ == '__main__':
  ...
  parser.add_argument('-a', '--filter1',type=int,default=3,help='first filter size');
  parser.add_argument('-b','--filter2',type=int,default=3,help='second filter size');
  parser.add_argument('-d','--dense',type=int,default=512,help='dense bank');
